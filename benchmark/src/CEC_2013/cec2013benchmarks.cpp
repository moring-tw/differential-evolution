#pragma warning(disable : 4996) // this code has some insecure issue, for more detail, see: https://docs.microsoft.com/zh-tw/cpp/error-messages/compiler-warnings/compiler-warning-level-3-c4996.

#include "../../inc/CEC_2013/cec2013benchmarks.h"
#include <WINDOWS.H>
#include <stdio.h>
#include <math.h>
#include <malloc.h>
...
CEC2013Benchmarks::CEC2013Benchmarks(){

}

CEC2013Benchmarks::CEC2013Benchmarks(int dim, int funcID){
    CEC2013Benchmarks::dim = dim;
    CEC2013Benchmarks::funcID = funcID;
    Benchmarks::ID = funcID;
}
...
double CEC2013Benchmarks::compute(double *x){
    double result;
	double* f = (double*)malloc(sizeof(double) * 1);
    test_func(x, f, CEC2013Benchmarks::dim,1,CEC2013Benchmarks::funcID);
    result = f[0];
    update(result);
    free(f);
    return result;
...
void CEC2013Benchmarks::test_func(double *x, double *f, int nx, int mx,int func_num)
{
	//
	// nx  = number of dimensions
	// mx = how many individuals needed to be calculated.
	//
	int cf_num=10,i;
	if (ini_flag==1)
	{
		...
		{
			// if numbers of dimensions or problem function had been changed, needs to read new setting of m, Oshift, y, z, x_bound
			ini_flag=0;
		}
	}
...
void CEC2013Benchmarks::sphere_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Sphere */
{
	int i;
	shiftfunc(x, y, nx, Os);
	if (r_flag==1)
...
void CEC2013Benchmarks::ellips_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Ellipsoidal */
{
    int i;
	shiftfunc(x, y, nx, Os);
	if (r_flag==1)
...
void CEC2013Benchmarks::bent_cigar_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Bent_Cigar */
{
    int i;
	double beta=0.5;
	shiftfunc(x, y, nx, Os);
...
void CEC2013Benchmarks::discus_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Discus */
{
    int i;
	shiftfunc(x, y, nx, Os);
	if (r_flag==1)
...
void CEC2013Benchmarks::dif_powers_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Different Powers */
{
	int i;
	shiftfunc(x, y, nx, Os);
	if (r_flag==1)
...
void CEC2013Benchmarks::rosenbrock_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Rosenbrock's */
{
    int i;
	double tmp1,tmp2;
...
void CEC2013Benchmarks::schaffer_F7_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Schwefel's 1.2  */
{
    int i;
	double tmp;
	shiftfunc(x, y, nx, Os);
...
void CEC2013Benchmarks::ackley_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Ackley's  */
{
    int i;
    double sum1, sum2;
...
void CEC2013Benchmarks::weierstrass_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Weierstrass's  */
{
    int i,j,k_max;
...
void CEC2013Benchmarks::griewank_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Griewank's  */
{
    int i;
    double s, p;
...
void CEC2013Benchmarks::rastrigin_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Rastrigin's  */
{
    int i;
...
void CEC2013Benchmarks::step_rastrigin_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Noncontinuous Rastrigin's  */
{
    int i;
	double alpha=10.0,beta=0.2;
	shiftfunc(x, y, nx, Os);
...
void CEC2013Benchmarks::schwefel_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Schwefel's  */
{
    int i;
	double tmp;
	shiftfunc(x, y, nx, Os);
...
void CEC2013Benchmarks::katsuura_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Katsuura  */
{
    int i,j;
...
void CEC2013Benchmarks::bi_rastrigin_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Lunacek Bi_rastrigin Function */
{
    int i;
...
void CEC2013Benchmarks::grie_rosen_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Griewank-Rosenbrock  */
{
    int i;
...
void CEC2013Benchmarks::escaffer6_func (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Expanded Scaffer's F6  */
{
    int i;
    double temp1, temp2;
	shiftfunc(x, y, nx, Os);
...
void CEC2013Benchmarks::cf01 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 1 */
{
	int i,cf_num=5;
	double fit[5];
...
void CEC2013Benchmarks::cf02 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 2 */
{
	int i,cf_num=3;
	double fit[3];
...
void CEC2013Benchmarks::cf03 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 3 */
{
	int i,cf_num=3;
	double fit[3];
	double delta[3] = {20,20,20};
...
void CEC2013Benchmarks::cf04 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 4 */
{
	int i,cf_num=3;
	double fit[3];
	double delta[3] = {20,20,20};
...
void CEC2013Benchmarks::cf05 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 4 */
{
	int i,cf_num=3;
	double fit[3];
...
void CEC2013Benchmarks::cf06 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 6 */
{
	int i,cf_num=5;
	double fit[5];
...
void CEC2013Benchmarks::cf07 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 7 */
{
	int i,cf_num=5;
	double fit[5];
...
void CEC2013Benchmarks::cf08 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 8 */
{
	int i,cf_num=5;
	double fit[5];
	double delta[5] = {10,20,30,40,50};
...
void CEC2013Benchmarks::shiftfunc (double *x, double *xshift, int nx,double *Os)
{
	int i;
    for (i=0; i<nx; i++)
    {
...
void CEC2013Benchmarks::rotatefunc (double *x, double *xrot, int nx,double *Mr)
{
	int i,j;
    for (i=0; i<nx; i++)
    {
...
void CEC2013Benchmarks::asyfunc (double *x, double *xasy, int nx, double beta)
{
	int i;
    for (i=0; i<nx; i++)
    {
...
void CEC2013Benchmarks::oszfunc (double *x, double *xosz, int nx)
{
	int i,sx;
	double c1,c2,xx = 0;
    for (i=0; i<nx; i++)
...
void CEC2013Benchmarks::cf_cal(double *x, double *f, int nx, double *Os,double * delta,double * bias,double * fit, int cf_num)
{
	int i,j;
	double *w;
